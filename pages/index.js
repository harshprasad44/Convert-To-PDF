...
const DocxToPdf = ({ secret }) => {
  ...
  const submitHandler = async (e) => {
    ...
    try {
      ...
      // const { data } = await axios.get(`/api/docx-to-pdf/${fileId}/${FileName}`);
      ...
      setConvertedUrl(data.Files[0].Url);
    ...
    <>
      <FormContainer>
        <form onSubmit={submitHandler}>
          {conversionError && <Message variant="danger">Oops! There was error converting the file. Please check the file and try again.</Message>}

          <input type="file" accept=".docx" id="file" name="File" onChange={uploadFileHandler} required />

          {fileLoading && <Loader />}
          {fileUploadError && <Message variant="danger">Oops! There was error uploading the file. Please check the file and try again.</Message>}
